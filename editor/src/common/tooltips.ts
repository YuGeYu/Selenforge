...
import { openBrandLink, sanitizeBrandUrl } from './brand';
...
export const tooltipRefItem = ({
    ...
    const btnUrl = new Button({
        ...
        text: 'API REFERENCE',
        flexGrow: 1,
        hidden: !sanitizeBrandUrl(reference.url)
    });
    if (sanitizeBrandUrl(reference.url)) {
        btnUrl.on('click', () => {
            openBrandLink(reference.url, `${reference.title} API`);
        });
    }
    item.append(btnUrl);
